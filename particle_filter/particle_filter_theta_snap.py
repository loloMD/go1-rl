...
"""
class LocalizationFilter_theta_snap:
    ...
    def predict_step(self, delta_action, delta_theta): 
        """
    ...
    def update_step(self, estimated_robot_position, theta_position): 
        """
    ...
    def get_robot_position(self): 
        ...
        full_pose = np.array([pos[0], pos[1], self.theta])
        return full_pose
        # return np.mean(self.particles*self.particle_weights.reshape((-1, 1)), axis=0)

    def predict_update_position(self, delta_action, estimated_robot_position, delta_theta, theta_position):
        """
        Predict and update the robot position
        """
        self.predict_step(delta_action, delta_theta)
        self.update_step(estimated_robot_position, theta_position)

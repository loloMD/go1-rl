...
import numpy as np 
from particle_filter_theta import LocalizationFilter_theta
...
if __name__ == "__main__": 
    ...
    init_robot_position = [1, 2, 0]

    filter = LocalizationFilter_theta(init_robot_position=init_robot_position, 
                                num_particles=4000)
    ...
    for timestep in range(100):
        ...
        true_action = np.random.random(3)
        true_action = (true_action * 2) - 1
        true_action[0] *= 2
        true_action[1] *= 2
        true_action[2] = np.random.uniform(0, 2*np.pi) * 1e-2

        # predict step 
        filter.predict_step(true_action)
        ...
        filter.update_step(observed_state)
